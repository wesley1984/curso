...
class Blog extends Admin
{
    ...
    public function home(?array $data): void
    {
        ...
        $search = null;
        $posts = (new Post())->findJoin("", "",
            "posts.*, usuario.name, usuario.sobrenome",

            "LEFT JOIN usuario ON usuario.id = posts.author");

        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            $search = str_search($data["search"]);
            $posts = (new Post())->findJoin("MATCH(title, subtitle) AGAINST(:s)", "s={$search}",
                "posts.*, usuario.name, usuario.sobrenome",
